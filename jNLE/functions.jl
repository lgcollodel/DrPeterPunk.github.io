...
# For multiple conditions for a single field the matrix is still sparse but no longer block-banded
function boundary_conditions!(res,u,ux,Nlr,p)
    c=zeros(sum(Nlr)); #c=zeros(Nlr[1]+Nlr[2]);
    Nx=length(u[:,1])
    for i=1:Nlr[1]
        j=(i-1)*Nx+1
        ubc!(c,u[1,:],ux[1,:],p)
    ...
    for i=1:Nlr[2]#i=Nlr[1]+1:Nlr[2]
        ii=i+Nlr[1]
        ubc!(c,u[end,:],ux[end,:],p)
        res[i*Nx]=c[ii]
    end
end
function jac!(j,u,ux,uxx,M,p,x,Nlr)
    Nx   = length(x)
    Nk   = length(u0[1,:])
    Ju   = j.Ju
    Jux  = j.Jux
...
function main(Nd, Px, x, u0, p, Nlr; r_tol=1.e-8)
    i0=0
    on=1.0
    Nk = length(u0[1,:])
    Nx = length(u0[:,1])
    ...
    jac!(j,u.u0,du0.ux,du0.uxx,M,p,x,Nlr)
    for i0=1:dform.Nx
    ...
    boundary_conditions!(eq0.Res,u0,du0.ux,Nlr,p)
    ...
    while rmax > r_tol
        ...
        Nk > 1 ? u.u1=reshape(vn,(dform.Nx,2)) : u.u1 = vn
        for k=1:Nk
        ...
        boundary_conditions!(eq1.Res,u.u1,du1.ux,Nlr,p)
        ...
        elseif (rmaxn < rmax) && (rmaxn > r_tol)
            ...
            v0=vn
            eq0.Res=eq1.Res
            jac!(j,u.u1,du1.ux,du1.uxx,M,p,x,Nlr)
        elseif (rmaxn < r_tol)
            u.u0=u.u1;
            du0=du1;
            #println("Convergence")
